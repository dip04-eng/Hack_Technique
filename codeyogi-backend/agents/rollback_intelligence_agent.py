    ...
    def execute_rollback(
        ...
        try:
            ...
            if create_rollback_commit:
                # Method 1: Create a revert commit (safe, preserves history)
                # This reverts all commits between current and target
...
"""
                ...
                # Note: GitHub API doesn't support revert directly
                # We return instructions for git operations that can be executed via GitHub Actions
                ...
                return {
                    ...
                    "git_instructions": {
                        "commands": [
                            f"git revert --no-commit {current_sha}..{target_sha}",
                            "git commit -m 'Rollback via CodeYogi'",
                            f"git push origin {branch}"
                        ],
                        "description": "Execute these commands to complete rollback"
                    },
                    "webhook_url": f"https://api.github.com/repos/{repo_owner}/{repo_name}/dispatches",
                    "workflow_dispatch": {
                        "event_type": "rollback",
                        "client_payload": {
